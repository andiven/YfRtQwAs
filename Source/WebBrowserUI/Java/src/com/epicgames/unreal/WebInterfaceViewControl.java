...
class WebInterfaceViewPositionLayout extends ViewGroup
{
	public WebInterfaceViewPositionLayout(Context context, WebInterfaceViewControl inWebViewControl)
	{
        super(context);
	...
	private WebInterfaceViewControl webViewControl;
}

// Wrapper for the layout and WebView for the C++ to call
class WebInterfaceViewControl
{

	private static int initialWidth = 500;
	private static int initialHeight = 500;
	private static final String TAG = "WebInterfaceViewControl";
	...
	public class InterceptedResponseHeaders 
	{
		...
		public InterceptedResponseHeaders()
		{
			MimeType = "text/html";
			Encoding = "utf-8";
		}
	...
	public WebInterfaceViewControl(long inNativePtr, int width, int height, boolean swizzlePixels, boolean vulkanRenderer, final boolean bEnableRemoteDebugging, final boolean bUseTransparency, final boolean bEnableDomStorage, final boolean bShouldUseBitmapRender)
	{
		final WebInterfaceViewControl w = this;
		...
		//GameActivity.Log.debug("WebIntefaceViewControl width=" + width + ", height=" + height);
		...
		GameActivity._activity.runOnUiThread(new Runnable()
		{
			...
			public void run()
			{
				...
				positionLayout = new WebInterfaceViewPositionLayout(GameActivity._activity, w);
	...
	public void setSize(int width, int height)
	{
		...
		{
			mOESTextureRenderer.setSize(width, height);
		}
		else if (null != mBitmapRenderer)
		{
			mBitmapRenderer.setSize(width, height);
		}
	}
...
*/
		...
		implements android.graphics.SurfaceTexture.OnFrameAvailableListener
	{
		...
		public BitmapRenderer(boolean swizzlePixels, boolean isVulkan)
		{
			...
			else
			{
				...
				if (RendererString.contains("Adreno (TM) "))
				{
					...
					{
						GameActivity.Log.debug("WebInterfaceViewControl: disabled shared GL context on " + RendererString);
		...
		private void showGlError(String op, int error)
		{
			switch (error)
			{
				case GLES20.GL_INVALID_ENUM:						GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_INVALID_ENUM");  break;
				case GLES20.GL_INVALID_OPERATION:					GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_INVALID_OPERATION");  break;
				case GLES20.GL_INVALID_FRAMEBUFFER_OPERATION:		GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_INVALID_FRAMEBUFFER_OPERATION");  break;
				case GLES20.GL_INVALID_VALUE:						GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_INVALID_VALUE");  break;
				case GLES20.GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:	GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT");  break;
				case GLES20.GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS:	GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS");  break;
				case GLES20.GL_FRAMEBUFFER_UNSUPPORTED:				GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_FRAMEBUFFER_UNSUPPORTED");  break;
				case GLES20.GL_OUT_OF_MEMORY:						GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError GL_OUT_OF_MEMORY");  break;
				default:											GameActivity.Log.error("WebInterfaceViewControl$BitmapRenderer: " + op + ": glGetError " + error);
			}
		}
	...
	*/
		...
		extends WebViewClient
	{
		...
		{
			InterceptedResponseHeaders Response = new InterceptedResponseHeaders();

			byte[] Data = shouldInterceptRequestImpl(Url, Response);
			if (Data != null)
			{
				...
				else if (Response.ReasonPhrase != null && Response.ReasonPhrase.length() > 0)
				{
					return new WebResourceResponse(Response.MimeType, Response.Encoding, Response.StatusCode, Response.ReasonPhrase, Response.Headers, new ByteArrayInputStream(Data));
				}
				else
				{
					return new WebResourceResponse(Response.MimeType, Response.Encoding, new ByteArrayInputStream(Data));
				}
			}
			else
			{
		...
		private native byte[] shouldInterceptRequestImpl(String Url, InterceptedResponseHeaders Response);

		public long GetNativePtr()
		{
			return WebInterfaceViewControl.this.nativePtr;
		}
	}
	...
	private WebInterfaceViewPositionLayout positionLayout;
	...
	// Address of the native SAndroidWebInterfaceBrowserWidget object
