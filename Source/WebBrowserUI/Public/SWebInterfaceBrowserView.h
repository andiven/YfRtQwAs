...
DECLARE_DELEGATE_TwoParams(FOnConsoleLogDelegate, const FString& /*Text*/, FColor /*Color*/);
...
public:
	...
	{ }
...
#if WITH_CEF3
		/** Called when the browser loses the mouse. */
		SLATE_EVENT(FOnMouseLost, OnMouseLost)
#endif
		...
		/** Called for each console log */
		SLATE_EVENT(FOnConsoleLogDelegate, OnConsoleLog)
...
private:
	...
	/** Callback when the mouse is lost */
...
private:
...
#if WITH_CEF3
	/** A delegate that is invoked when the browser loses the mouse */
	FOnMouseLost OnMouseLost;
#endif

	/** A delegate that is invoked when the browser needs to present a dialog to the user */
	FOnShowDialog OnShowDialog;

	/** A delegate that is invoked when the browser needs to dismiss all dialogs */
	FSimpleDelegate OnDismissAllDialogs;

	FOnSuppressContextMenu OnSuppressContextMenu;

	/** A delegate that is invoked when the browser wishes to create a tooltip */
	...
	/** A delegate that is invoked for each console log */
	FOnConsoleLogDelegate OnConsoleLog;
