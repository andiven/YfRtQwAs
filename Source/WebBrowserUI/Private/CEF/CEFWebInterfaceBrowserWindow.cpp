...
#define UI_CONSOLE(ColorName, FormatString, ...)	ConsoleLog( FColor::ColorName, TEXT( FormatString ), ##__VA_ARGS__ )
...
#endif
{
	...
	UpdatableTextures[0].Reset();
	UpdatableTextures[1].Reset();
...
void FCEFWebInterfaceBrowserWindow::ReleaseTextures()
{
	for (int I = 0; I < 2; ++I)
	{
		FCEFWebInterfaceBrowserWindowTexture& UpdatableTexture = UpdatableTextures[I];
		if (UpdatableTexture.IsValid())
			UpdatableTexture.ResetSlateTexture();
	}
}
...
bool FCEFWebInterfaceBrowserWindow::CreateInitialTextures()
{
	if (FSlateApplication::IsInitialized())
	{
		if (FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer())
		{
			...
			UpdatableTextures[0].GetSlateTexture()->UpdateTextureThreadSafeRaw(1, 1, RawData.GetData());
			...
			UpdatableTextures[1].GetSlateTexture()->UpdateTextureThreadSafeRaw(1, 1, RawData.GetData());
...
TOptional<FSlateRenderTransform> FCEFWebInterfaceBrowserWindow::GetWebBrowserRenderTransform() const
{
	...
	if (bUsingAcceleratedPaint && RHIRenderHelper != nullptr)
	{
...
TOptional<FScale2D> FCEFWebInterfaceBrowserWindow::GetWebBrowserRenderScale() const
{
	TOptional<FScale2D> LocalRenderScale;
	if (bUsingAcceleratedPaint && RHIRenderHelper != nullptr)
	{
		LocalRenderScale = RHIRenderHelper->GetWebBrowserRenderScale();
	}
	return LocalRenderScale;
}
...
{
	if (UpdatableTextures[bIsPopup?1:0].IsValid())
	{
...
bool FCEFWebInterfaceBrowserWindow::IsUsingAcceleratedPaint() const
{
	return bUsingAcceleratedPaint;
}

bool FCEFWebInterfaceBrowserWindow::HasAcceleratedPaintInterop() const
{
#if PLATFORM_WINDOWS
#if UE_VERSION >= 501
	if (bUsingAcceleratedPaint && RHIGetInterfaceType() == ERHIInterfaceType::D3D12)
		return UpdatableTextures[0].HasInterop();
#else
	static bool sbD3D12Renderer = TCString<TCHAR>::Stricmp(GDynamicRHI->GetName(), TEXT("D3D12")) == 0;
	if (bUsingAcceleratedPaint && sbD3D12Renderer)
		return UpdatableTextures[0].HasInterop();
#endif
#endif
	return false;
...
void FCEFWebInterfaceBrowserWindow::OnFocus(bool SetFocus, bool bIsPopup)
{
	...
	if (!SetFocus && OnMouseLost().IsBound())
	{
		OnMouseLost().Execute();
	}
}
...
void FCEFWebInterfaceBrowserWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
	...
	if (bUsingAcceleratedPaint)
	{
		UI_CONSOLE(Red, "Accelerated CEF rendering selected but OnPaint called. Disabling accelerated rendering for this browser window.");

		bUsingAcceleratedPaint = false;
		if (UpdatableTextures[Type].IsValid())
			UpdatableTextures[Type].ResetSlateTexture(AsShared());
	}
	...
	if (UpdatableTextures[Type].IsValid())
	{
		...
		{
			UpdatableTextures[Type].GetSlateTexture()->UpdateTextureThreadSafeRaw(Width, Height, Buffer, Dirty);
			HandleRenderingError();
...
void FCEFWebInterfaceBrowserWindow::OnAcceleratedPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, void* SharedHandle)
{
	...
	if (!bUsingAcceleratedPaint)
	{
		UI_CONSOLE(Red, "Accelerated CEF rendering NOT selected but OnAcceleratedPaint called. Enabling accelerated rendering for this browser window.");
	...
	if (!UpdatableTextures[Type].IsValid())
	{
		if (FSlateRenderer* const Renderer = GetRenderer())
		{
			if (RHIRenderHelper)
			{
				UpdatableTextures[Type] = RHIRenderHelper->CreateTexture(AsShared(), SharedHandle);
			}
			else
			{
#if UE_VERSION >= 500
	...
	if (UpdatableTextures[Type].IsValid())
	{
		...
		if (RHIRenderHelper)
		{
			FSlateUpdatableTexture* SlateTexture = UpdatableTextures[Type].GetSlateTexture();
			RHIRenderHelper->UpdateSharedHandleTexture(AsShared(), SharedHandle, UpdatableTextures[Type], Dirty.Scale(ViewportDPIScaleFactor));
			if (SlateTexture && SlateTexture != UpdatableTextures[Type].GetSlateTexture())
			{
				if (FSlateRenderer* const Renderer = GetRenderer())
				{
					Renderer->ReleaseUpdatableTexture(SlateTexture);
					HandleRenderingError();
				}
			}
		}
		...
		{
#if UE_VERSION >= 500
			UpdatableTextures[Type].GetSlateTexture()->UpdateTextureThreadSafeWithKeyedTextureHandle(SharedHandle, 1, 0, Dirty.Scale(ViewportDPIScaleFactor));
#endif
		}
...
void FCEFWebInterfaceBrowserWindow::ConsoleLog(const FString& Text, FColor Color)
{
	if (Color == FColor::Red)
	{
		UE_LOG(LogWebInterfaceBrowser, Error, TEXT("%s"), *Text);
	}
	else if (Color == FColor::Yellow)
	{
		UE_LOG(LogWebInterfaceBrowser, Warning, TEXT("%s"), *Text);
	}
	else
	{
		UE_LOG(LogWebInterfaceBrowser, Log, TEXT("%s"), *Text);
	}

	ConsoleLogDelegate.ExecuteIfBound(Text, Color);
}

void FCEFWebInterfaceBrowserWindow::UpdateVideoBuffering()
{
	if (BufferedVideo.IsValid() && UpdatableTextures[PET_VIEW].IsValid())
	{
		FSlateTextureData* SlateTextureData = BufferedVideo->GetNextFrameTextureData();
		if (SlateTextureData != nullptr)
		{
			UpdatableTextures[PET_VIEW].GetSlateTexture()->UpdateTextureThreadSafeWithTextureData(SlateTextureData);
			HandleRenderingError();
		}
	}
}
...
#define IDC_PAN_WEST 25856
	...
	HINSTANCE CEF3ModuleHandle = (HINSTANCE)WebBrowserUtils::GetCEF3ModuleHandle();
...
EWebInterfaceBrowserConsoleLogSeverity CefLogSeverityToWebInterfaceBrowser(cef_log_severity_t Level)
{
	switch (Level)
	{
...
{
	ConsoleMessageDelegate.ExecuteIfBound(WCHAR_TO_TCHAR(Message.ToWString().c_str()), WCHAR_TO_TCHAR(Source.ToWString().c_str()), Line, CefLogSeverityToWebInterfaceBrowser(Level));
}
...
CefMouseEvent FCEFWebInterfaceBrowserWindow::GetCefMouseEvent(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	...
	if (bUsingAcceleratedPaint)
	{
		TOptional<FScale2D> LocalRenderScale = GetWebBrowserRenderScale();
		if (LocalRenderScale.IsSet())
			MouseGeometry = MyGeometry.MakeChild(FSlateRenderTransform(LocalRenderScale.GetValue()));
	}
