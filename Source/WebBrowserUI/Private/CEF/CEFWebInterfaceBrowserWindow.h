...
public:
	...
	virtual bool IsUsingAcceleratedPaint() const override;
	virtual bool HasAcceleratedPaintInterop() const override;
	...
	virtual FOnMouseLost& OnMouseLost() override
	{
		return MouseLostDelegate;
	}
...
public:
	...
	* Log to console.
	*/
	void ConsoleLog(const FString& Text, FColor Color = FColor::White);

	template <typename FmtType>
	void ConsoleLog( FColor Color, const FmtType& Fmt )
	{
		ConsoleLog( FString( Fmt ), Color );
	}

	template <typename FmtType, typename... Types>
	void ConsoleLog( FColor Color, const FmtType& Fmt, Types... Args )
	{
		ConsoleLog( FString::Printf( Fmt, Args... ), Color );
	}
...
private:
	...
	FCEFWebInterfaceBrowserWindowTexture UpdatableTextures[2];
	...
	/** Delegate that allows for response to console logs. Typically used to capture and mirror engine logs in client application logs. */
	FOnConsoleLogDelegate ConsoleLogDelegate;

	/** Delegate that allows for response to console messages. Typically used to capture and mirror web logs in client application logs. */
	...
	/** Delegate for losing the mouse. */
	FOnMouseLost MouseLostDelegate;
