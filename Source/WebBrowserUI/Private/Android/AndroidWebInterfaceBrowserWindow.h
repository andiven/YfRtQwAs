...
*/
class FAndroidWebInterfaceBrowserWindow
	: public IWebInterfaceBrowserWindow
	, public TSharedFromThis<FAndroidWebInterfaceBrowserWindow>
{
	// The WebInterfaceBrowserSingleton should be the only one creating instances of this class
	friend class FWebInterfaceBrowserSingleton;
	// CreateWidget should only be called by the WebInterfaceBrowserView
...
private:
	 ...
	 */
	FAndroidWebInterfaceBrowserWindow(FString Url, TOptional<FString> ContentsToLoad, bool bShowErrorMessage, bool bThumbMouseButtonNavigation, bool bUseTransparency, bool bJSBindingToLoweringEnabled);

	/**
	 * Create the SWidget for this WebInterfaceBrowserWindow
	 */
	TSharedRef<SWidget> CreateWidget();

public:
	/** Virtual Destructor. */
	virtual ~FAndroidWebInterfaceBrowserWindow();

public:

	// IWebInterfaceBrowserWindow Interface
	...
	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnDocumentStateChanged, FOnDocumentStateChanged);
	virtual FOnDocumentStateChanged& OnDocumentStateChanged() override
	{
		return DocumentStateChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnTitleChanged, FOnTitleChanged);
	virtual FOnTitleChanged& OnTitleChanged() override
	{
		return TitleChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnUrlChanged, FOnUrlChanged);
	virtual FOnUrlChanged& OnUrlChanged() override
	{
		return UrlChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnToolTip, FOnToolTip);
	virtual FOnToolTip& OnToolTip() override
	{
		return ToolTipEvent;
	}

	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnNeedsRedraw, FOnNeedsRedraw);
	...
	virtual FOnConsoleLogDelegate& OnConsoleLog() override
	{
		return ConsoleLogDelegate;
	}
	...
	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnShowPopup, FOnShowPopup);
	virtual FOnShowPopup& OnShowPopup() override
	{
		return ShowPopupEvent;
	}

	DECLARE_DERIVED_EVENT(FAndroidWebInterfaceBrowserWindow, IWebInterfaceBrowserWindow::FOnDismissPopup, FOnDismissPopup);
...
public:
	...
	/**
	* Called from the WebInterfaceBrowserSingleton tick event. Should test whether the widget got a tick from Slate last frame and set the state to hidden if not.
	*/
...
private:
	...
	TSharedPtr<SAndroidWebInterfaceBrowserWidget> BrowserWidget;
	...
	/** Delegate that is invoked for each console log */
	FOnConsoleLogDelegate ConsoleLogDelegate;
	...
	FMobileInterfaceJSScriptingPtr Scripting;
...
typedef FAndroidWebInterfaceBrowserWindow FWebInterfaceBrowserWindow;
